        ...
        <div>
            ...
            <ul style={{ margin: 0, padding: 0, listStyleType: "none" }}>
                {store.snapshots.map((snapshot, i) => (
                    <li key={`snapshot${i}`} className="flex" style={{ margin: "0 0 8px 0", padding: 0 }}>
                        <span style={{ minWidth: "20px" }}><strong>{i + 1}</strong></span>
                        <span style={{ flexGrow: 1 }}>{truncateString(JSON.stringify(store.snapshots[i]), 26)}</span>
                        <span>
