...
export const parseJSONConfig = (config, data) => {
  ...
  const parsedConfig = config.map((item) => {
    ...
    if (newItem.computedValue && typeof newItem.computedValue === "string") {
      ...
      try {
        computedValueFunction = new Function(
          "data, itemData, interfaceState",
          `"use strict"; let result; try { result = ${newItem.computedValue}; } catch (e) { console.warn("error in computed function"); } return result;`
        );
      ...
      if (typeof computedValueFunction === "function")
        newItem.computedValue = computedValueFunction;
    }
    if (newItem.isRendered && typeof newItem.isRendered === "string") {
      let isRenderedFunction;
      try {
        isRenderedFunction = new Function(
          "path, fieldData, data, interfaceState",
          `"use strict"; let result = true; try { result = ${newItem.isRendered}; } catch (e) { console.warn("error in isRendered function"); } return result;`
        );
      ...
      // eslint-disable-next-line no-new-func
      if (typeof isRenderedFunction === "function")
        newItem.isRendered = isRenderedFunction;
    }
    return newItem;
  });
  return parsedConfig;
...
export const getConfigPathFromDataPath = (path, config) => {
  ...
  const pathSplit = path.startsWith("{")
    ? path.substring(path.indexOf("}") + 2).split(".")
    : path.split(".");
  let realPath = "";
  pathSplit.forEach((key) => {
    const index = _.findIndex(realPath ? _.get(config, realPath) : config, {
      id: key.replace(/\[(\d+)\]/, ""),
    });
  ...
  return realPath.endsWith(".fields") || realPath.endsWith(".stages")
    ? realPath.substring(0, realPath.length - 7)
    : realPath;
};
...
export const createNewFieldID = (path, type, store) => {
  ...
  while (
    doesPathExist(
      parentPath ? `${parentPath}.${newFieldID}` : newFieldID,
      store
    )
  ) {
    if (counter) {
    ...
    config.forEach((item) => {
      if (item) {
        ...
        if (
          ...
          paths = paths.concat(
            getAllPaths(item.fields, path ? `${path}.${item.id}` : item.id)
          );
        }
      }
    });
...
export const pathIsSelected = (path, selectedElement, fieldsetId) => {
  ...
  return (
    selectedElement === actualPath ||
    (Array.isArray(selectedElement) && selectedElement.includes(actualPath))
  );
};
...
function removeEmptyArraysFromObject(obj) {
  ...
  for (let key in newObj) {
    if (
      ...
      (newObj[key].length === 0 || _.isEmpty(newObj[key][0]))
    ) {
      delete newObj[key];
