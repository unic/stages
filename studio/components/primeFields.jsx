  ...
  if (isInInspector) {
    return (
      <div
        ...
        style={
          isDisabled
            ? { opacity: 0.5, pointerEvents: "none", padding: 0 }
            : { padding: 0 }
        }
      >
        <div style={{ flexGrow: 1, minWidth: "140px", maxWidth: "140px" }}>
          <label htmlFor={id}>
            {parseTemplateLiterals(label, store.data)}
            {isRequired ? " *" : ""}
          </label>
        </div>
        <div>&nbsp;</div>
    ...
    <div
      ...
      style={
        isDisabled
          ...
          : { minWidth: "auto", marginBottom: 0 }
      }
    >
      <label
        htmlFor={id}
        style={{
          userSelect: store.isEditMode ? "none" : "auto",
          cursor: "text",
        }}
      >
        <span
      ...
      {secondaryText ? (
        <div style={{ margin: "-8px 0 8px 0", color: "#999" }}>
          <span
...
const MappedHeading = (props) => {
  ...
  const [text, setText] = React.useState(
    props.text ? parseTemplateLiterals(props.text, store.data) : ""
  );
    ...
    <div>
      {(!props.level || props.level === 2) && (
        <h2 style={{ marginTop: 0 }}>
          <span
            contentEditable={!textHasTemplateLiterals(props.title)}
      ...
      {props.text && (
        <p style={{ color: "#999", marginTop: "-12px", marginBottom: "4px" }}>
          <span
            contentEditable={!textHasTemplateLiterals(props.text)}
...
const MappedMessage = (props) => {
  return <Message text={props.text} severity={props.severity} />;
};
...
const MappedSelectButton = (props) => {
  ...
  if (typeof mappedProps.value === "undefined")
    mappedProps.value = mappedProps.defaultValue || "";
  mappedProps.onChange = (e) => {
    props.onChange(e.target.value);
  };
  mappedProps.optionLabel = "text";
...
const MappedCalendar = (props) => {
  ...
  if (typeof mappedProps.value === "undefined") {
    ...
    mappedProps.value =
      typeof mappedProps.value === "string"
        ? new Date(mappedProps.value)
        : mappedProps.value;
  }
  mappedProps.onChange = (e) => {
    props.onChange(e.target.value);
...
const MappedInputNumber = (props) => {
  const mappedProps = removeStagesProps(props);
  if (typeof mappedProps.value === "undefined" || isNaN(mappedProps.value))
    mappedProps.value = 0;
  mappedProps.type = "text";
  mappedProps.onChange = (e) => {
    props.onChange(e.value);
...
const MappedDivider = (props) => {
  return (
    <Divider
      ...
      type={props.borderType || props.isInInspector ? "solid" : "dashed"}
      layout={props.layout || "horizontal"}
      align={props.align || "center"}
    >
      {props.text && (
        <span style={{ color: "#999", fontSize: "12px" }}>{props.text}</span>
      )}
    </Divider>
  );
};
