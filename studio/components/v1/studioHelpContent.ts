...
export const studioHelpTopics: readonly StudioHelpTopic[] = [
  ...
  { title: "Layers & fragments", summary: "Layers shows the form as a list, including items nested inside other items.", fields: [
    ...
    ["Design keyboard shortcuts", "With the canvas focused, use Command (Mac) or Control (Windows/Linux) + C/X/V to copy/cut/paste, D to duplicate, G to group, Shift+G to ungroup, A to select all root items, Z to undo, and Shift+Z or Y to redo. Backspace cuts the selection; Delete deletes it. Escape clears selection. Up/Down reorder adjacent selected siblings; Right moves them into the previous container and Left moves them out. In Layers, use Alt+arrows to move; plain arrows navigate the tree. Shortcuts leave typing in fields, inspector controls, and dialogs unchanged. Paste with no selection adds items at the form root."],
    ["Canvas borders / S M L", "Design borders show the item type and runtime path (or the content item ID). Hover, focus, or select an editable item to use S, M, or L for quarter, half, or full width at the toolbar’s selected breakpoint. Other breakpoints keep their widths. Layout changes support undo and redo. Preview hides these controls."],
    ...
    ["Create fragment from selection / Insert fragment", "Save contiguous sibling items as a reusable group from Layers, Insert, or the context menu. Insert linked copies inside a selected container or after a selected item. Canvas insertion menus offer saved fragments too."],
    ["Edit shared contents", "Choose Edit in the Layers fragment list or Edit shared contents on an instance. Select a shared item to change its properties, validation, or logic for every linked instance. Each instance keeps independent values. Back to form returns to instance settings."],
    ["Label overrides / Detach instance", "Override a label for one instance; clear it to use the shared label again. Detach converts the instance to an ordinary editable group. Shared edits, overrides, and detachment support undo and redo."],
