...
| Phase 8 — Hardening and v1 release | Partial | Strict builds, compile-time fixtures, SSR-safe core boundaries, async race tests, mutation checks, controller-isolation tests, seeded immutable-path, collection-command, and mixed controller transaction properties, formal initialization/batch/selector performance budgets, tarball allowlist/export checks, offline packed installation, runtime import smoke tests, packed declaration consumption, and packed recreation exist. | Broaden malformed-schema fuzzing, complete API and 0.x migration documentation, release packaging, and release-candidate validation. |
...
- `npm run test:v1` builds the packages and currently runs 72 passing executable
