...
Controller state-machine properties now run 1,200 seeded mixed transactions
across controlled input, owner updates, explicit batching, context-driven schema
changes, interaction metadata, and collection add/remove/move commands while
checking an independent canonical model and snapshot invariants after every
step.
Malformed-schema properties now normalize 1,000 seeded nested invalid trees and
apply 180 invalid dynamic revisions. They require deterministic diagnostics,
frozen-input preservation, last-valid-tree retention, and recovery when the
schema becomes valid again.
The implemented public surface is now documented in `V1_API.md`, including the
controlled callback order, schema/events, collections, wizards, validation,
serialization, and adapter contracts. Every package ships a focused README, and
the tarball verifier requires it alongside the runtime and declaration entries.
An exhaustive structural matrix now covers all 84 group, homogeneous
collection, discriminated collection, and wizard nesting permutations through
...
| Phase 8 — Hardening and v1 release | Mostly implemented | Strict builds, compile-time fixtures, SSR-safe core boundaries, async race tests, mutation checks, controller-isolation tests, seeded immutable-path, collection-command, mixed controller transaction, and malformed-schema properties, formal initialization/batch/selector performance budgets, implementation-accurate API and migration documentation, package READMEs and licenses, complete publish metadata, navigable source maps, tarball allowlist/export checks, offline packed installation, runtime and declaration consumption, packed codec/migration/recreation, production example builds, an executable eighteen-criterion acceptance ledger, and a repeatable full release-candidate gate exist. | Exercise a selected immutable RC version with real external consumers and complete maintainer acceptance before publication. |
...
- A pure, immutable compatibility converter maps Studio groups, collections,
  ...
  values, computed values, and required validation to a v1 schema. It also
  prepares missing collection/wizard value containers for the controlled
  runtime. Computed expressions become ordered root transforms, including
  concrete immutable patches for fields inside nested collection rows.
- Invalid expressions and unknown field types are reported visibly as
  compatibility diagnostics rather than failing silently.
...
### Current documentation application
...
- `npm run check:v1` first verifies the API/migration documentation inventory,
  ...
  checking for all four packages and the vanilla and React examples, builds the
  packages' ESM declaration/output artifacts, and audits the evidence ledger for
  all eighteen architecture acceptance criteria.
...
- `npm run release:check:v1` composes the strict checks, package tests, Studio
  compatibility tests and production build, the documentation production
  build, vanilla and React production builds, and the React 19 lifecycle test
  into the documented release-candidate gate in `V1_RELEASE_CHECKLIST.md`.
- Generated package `dist/` directories are build artifacts and are not tracked.
- `V1_ACCEPTANCE_REVIEW.md` records the automated criterion evidence separately
  from the real external-consumer and maintainer approvals still required for
  stable promotion.
